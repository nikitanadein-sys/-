...
const cardClass =
  'group flex items-center gap-3 px-4 py-3 rounded-xl bg-white dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700 shadow-sm animate-fadein hover:ring-indigo-300 dark:hover:ring-indigo-600 transition'
  ...
  <div class="max-w-2xl mx-auto px-4 py-12 sm:py-16">
    ...
    <header class="flex items-center justify-between mb-8 animate-fadein">
      ...
      <button
        ...
        class="p-2.5 rounded-xl bg-white dark:bg-slate-800 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 hover:scale-105 active:scale-95 transition">
    ...
    <!-- форма добавления -->
    <form @submit.prevent="add" class="mb-6 flex gap-2 animate-pop">
      <input
        ref="input"
        v-model="newText"
        type="text"
        ...
        class="flex-1 px-4 py-3 rounded-xl bg-white dark:bg-slate-800 ring-1 ring-slate-200 dark:ring-slate-700
      ...
      <button
        type="submit"
        class="px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 text-white font-semibold
               shadow-lg shadow-indigo-500/30 hover:brightness-110 active:scale-95 transition whitespace-nowrap">
    ...
    <div v-show="total > 0" class="mb-5 animate-fadein">
      <div class="flex items-center justify-between text-sm text-slate-500 dark:text-slate-400 mb-1.5">
        <span>{{ statDone }}</span>
        <button
          v-show="doneCount > 0"
          @click="clearDone"
          class="text-indigo-600 dark:text-indigo-400 hover:underline underline-offset-2">Очистить выполненные</button>
      </div>
    ...
    <div class="flex items-center gap-1.5 mb-4">
      <button
        ...
        @click="filter = f.k"
        :class="[
          'fbtn flex items-center gap-1.5 pl-3.5 pr-2 py-1.5 text-sm rounded-full ring-1 transition',
          filter === f.k
            ? 'bg-indigo-600 text-white ring-transparent'
            : 'ring-slate-200 dark:ring-slate-700 bg-white dark:bg-slate-800 hover:ring-indigo-400',
        ]">
        {{ f.label }}
        <b
          v-show="total > 0"
          :class="[
            'min-w-[1.35rem] px-1.5 rounded-full text-[11px] leading-4 text-center',
            filter === f.k
              ? 'bg-indigo-500 text-white'
              : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300',
          ]">{{ counts[f.k] }}</b>
      </button>
      <span v-show="total > 0" class="ml-auto text-xs text-slate-400">{{ visible.length }} покан. задач</span>
    </div>
    ...
    <ul class="tasks space-y-2 max-h-[55vh] overflow-y-auto pr-1">
      <li
        v-for="t in visible"
        :key="t.id"
        :class="[cardClass, t.done && 'opacity-70', leavingId === t.id && 'leaving']">
        ...
        <button
          ...
          @click="startEdit(t)"
          title="Редактировать"
          class="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-slate-700 opacity-0 group-hover:opacity-100 transition">
        ...
        <button
          @click="remove(t.id)"
          title="Удалить"
          class="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-slate-700 opacity-0 group-hover:opacity-100 transition">
